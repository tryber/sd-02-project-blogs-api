...
describe('userController tests', () => {
  describe('Login User', () => {
    test('If pass invalid requisition, return a Joi message and status 422', async () => {
      ...
      const mockJoi = {
        code: 'invalid_data', error: true, message: '"email" must be a valid email',
      };
    ...
    test('If pass invalid credentials, return a service message and status 400', async () => {
      const mockBodyData = { email: 'johnatas@emai.com', password: '131072' };

      const mockReq = { body: mockBodyData };
      const mockJson = jest.fn();
      const mockRes = { status: jest.fn().mockReturnValueOnce({ json: mockJson }) };
      const mockNext = jest.fn();
      const mockServiceAnswer = {
        code: 'bad_request', error: true, message: 'Campos inválidos',
      };
  ...
  describe('Get User By Id', () => {
    test('If service does not find any info, return a message', async () => {
      ...
      const mockNext = jest.fn();
      const serviceAnswer = {
        code: 'not_found', error: true, message: 'Usuário não existe',
      };
